...
import { ArrowLeft, Cable, CheckCircle2, CircleAlert, Printer, RefreshCw, Server } from "lucide-react";
...
type Printer = { name: string; state: string; device: string | null; enabled: boolean; connected: boolean };
type PrinterStatus = { host: string; configuredPrinter: string; printers: Printer[] };
type SetupState = "checking" | "choose" | "testing" | "confirm" | "ready" | "error";

/** Connect the host printer, verify a physical test label, then print 30×50mm labels. */
export default function LabelPrintWorkspace() {
  ...
  const refreshPrinterStatus = useCallback(async () => {
    ...
    setSetupError("");
    try {
      const response = await fetch("/api/admin/printer", { cache: "no-store" });
      ...
      setSetupError(err instanceof Error ? err.message : "The host printer service is unavailable.");
      setSetupState("error");
    }
  }, []);
  ...
  async function printLabels() {
    ...
    try {
      const response = await fetch("/api/admin/labels", {
        ...
        body: JSON.stringify({ items, print: true, printer: selectedPrinter }),
      });
      const result = (await response.json().catch(() => ({}))) as { message?: string; printer?: string; jobId?: string; count?: number };
      if (!response.ok) throw new Error(result.message || "The printer could not be reached.");
      setNotice(`${result.count || "Label"} label${result.count === 1 ? "" : "s"} sent to ${result.printer || selectedPrinter} (${result.jobId || "job submitted"}).`);
    } catch (err) {
  ...
  async function testPrinter() {
    ...
    try {
      const response = await fetch("/api/admin/printer", {
        ...
        body: JSON.stringify({ action: "test", printer: selectedPrinter }),
      });
      ...
      setTestJob(result.jobId || "Job submitted"); setSetupState("confirm");
    } catch (err) {
  ...
  function confirmPrinter() {
    setLabelsUnlocked(true); setSetupState("ready"); setSetupError(""); setError(""); setNotice("");
  }
    ...
    <div className="mx-auto max-w-7xl space-y-8 pb-8">
      ...
      {!labelsUnlocked ? (
        <section className="mx-auto max-w-4xl rounded-3xl border border-black/[0.07] bg-white p-5 shadow-sm sm:p-8">
          ...
          {setupState === "checking" ? <div className="rounded-2xl border border-dashed border-black/15 px-5 py-10 text-center text-sm text-black/50">Checking the host printer service…</div> : setupState === "error" ? null : <>
            <div className="space-y-3"><div className="flex items-center justify-between gap-3"><div><h3 className="text-sm font-semibold">Available printers on {printerStatus?.host}</h3><p className="mt-1 text-xs text-black/50">Click a printer to use it for this session.</p></div><button type="button" onClick={() => void refreshPrinterStatus()} disabled={setupState === "testing"} className="inline-flex min-h-10 items-center gap-2 rounded-xl px-3 text-xs font-semibold text-[#28637d] hover:bg-[#f1f7f7] disabled:opacity-50"><RefreshCw size={14} />Refresh</button></div>
            ...
            <p className="mt-4 text-center text-xs leading-5 text-black/45">Labels stay locked until you confirm the physical output. This protects against a queue that accepts jobs while the printer is offline.</p>
          </>}
        </section>
      ) : (
