...
export default function CounterPrinterSetup({
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setMessage(error instanceof Error ? error.message : "The local printer connector is unavailable.");
      setState("error");
    }
  }, []);
  ...
  async function test() {
    ...
    try {
      ...
      const printed = await sendLocalPrintJob({ printer: selected, title: payload.title || "Baebe Boo counter printer connection test", jobBase64: payload.jobBase64 });
      setJobId(printed.jobId);
      setState("confirm");
    } catch (error) {
  ...
  return (
    <section className="rounded-2xl border border-[var(--color-line)] bg-[var(--color-surface)] p-4">
      ...
      {state === "checking" ? <p className="mt-4 rounded-xl border border-dashed border-[var(--color-line)] px-3 py-5 text-center text-xs text-[var(--color-ink-soft)]">Checking this workstation&apos;s local printer connector…</p> : state === "error" ? <div role="alert" className="mt-4 flex items-center justify-between gap-3 rounded-xl bg-red-50 px-3 py-2 text-xs font-semibold text-red-900"><span className="flex items-center gap-2"><CircleAlert size={15} />{message}</span><button type="button" onClick={() => void load()} className="inline-flex min-h-9 items-center gap-1 rounded-lg bg-white px-2"><RefreshCw size={13} />Retry</button></div> : <>
        ...
        {state === "confirm" ? <div className="mt-4 rounded-xl bg-emerald-50 p-3"><p className="flex items-center gap-2 text-xs font-semibold text-emerald-900"><CheckCircle2 size={15} />Receipt test sent · {jobId}</p><p className="mt-1 text-[11px] leading-5 text-emerald-900/75">Check the counter printer. Continue only if the 80 mm receipt test page physically printed.</p><div className="mt-3 flex gap-2"><button type="button" onClick={async () => { setState("testing"); setMessage(""); try { const response = await fetch("/api/counter/printer", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "verify", printer: selected, bridgeJobId: jobId }) }); const payload = (await response.json().catch(() => ({}))) as { message?: string }; if (!response.ok) throw new Error(payload.message || "The printer verification could not be saved."); onVerified(selected); } catch (error) { setMessage(error instanceof Error ? error.message : "The printer verification could not be saved."); setState("confirm"); } }} className="admin-button min-h-10 flex-1 text-xs">I see it — continue</button><button type="button" onClick={() => void test()} className="admin-button admin-button-secondary min-h-10 flex-1 text-xs">Test again</button></div></div> : <button type="button" onClick={() => void test()} disabled={!selected || state === "testing"} className="admin-button mt-4 min-h-11 w-full disabled:opacity-50"><Printer size={16} />{state === "testing" ? "Sending test…" : "Print receipt test page"}</button>}
      </>}
    </section>
  );
}
